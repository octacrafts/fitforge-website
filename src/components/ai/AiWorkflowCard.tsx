import type { AiWorkflowItem } from "@/lib/site/ai";
import {
  AI_WORKFLOW_TONE,
  AiWorkflowLeadIcon,
  AiWorkflowPreview,
  AiWorkflowWhyIcon,
} from "@/components/ai/AiWorkflowVisuals";

type AiWorkflowCardProps = Pick<
  AiWorkflowItem,
...
export default function AiWorkflowCard({
  ...
  const theme = AI_WORKFLOW_TONE[tone];
  ...
  return (
    <article className="flex h-full flex-col rounded-[24px] border border-card-line bg-card-dark p-6 shadow-[var(--shadow-card)] sm:p-7">
      <div className="flex items-start justify-between gap-3">
        ...
        >
          <AiWorkflowLeadIcon tone={tone} />
        </span>
        <AiWorkflowPreview item={{ tone, preview }} />
      </div>
